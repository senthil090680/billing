...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_sales where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum' and financialyear = '$financialyear'";
	...
	*/
	...
	$creditdays = $_REQUEST['creditdays'];
	...
	subtotaldiscountpercentapply1, subtotaldiscountamountapply1, subtotaldiscountamountonlyapply1, subtotaldiscountamountonlyapply2, creditdays) 	
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			if ($itemcode != '')
			{
				$query7 = "insert into sales_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
				itemrate, itemquantity, taxtype, 
				tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
				updatedate, companyanum, companyname, financialyear, amountbeforetax) 
				...
				'$amountaftertax', '$ipaddress', '$updatedatetime', '$companyanum', '$companyname', '$financialyear', '$itemtotalamount')";
				$exec7 = mysql_query($query7) or die ("Error in Query7".mysql_error());
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into sales_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
