...
if ($upload == 'success')
{
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$itemname = str_replace('\"\"', '"', $itemname); //To replace MS Excel given double quote to string.
			$itemname = str_replace('\"', '', $itemname); //To retain origina single char double quote and remeove Excel quote.
					...
					else
					{
						...
						else
						{
							...
							if ($rowcount5 != 0)
							{
								...
								$query2 = "insert into master_item (itemcode, itemname, categoryname, 
								unitname_abbreviation, rateperunit, ipaddress, updatetime, 
								taxanum, taxname, purchaseprice) 
