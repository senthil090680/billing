...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_dcsupplieroutward where billnumber = '$billnumber' and companyanum = '$companyanum'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_dcsupplieroutward where companyanum = '$companyanum'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_dcsupplieroutward (companyanum, billnumber, billdate, suppliertype, subtotal, suppliercode, suppliername, 
	...
	$query3 = "select auto_number from master_dcsupplieroutward  where companyanum = '$companyanum' and suppliercode = '$suppliercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into dcsupplieroutward_details (bill_autonumber, companyanum, 
			...
			values ('$itemcode', '$itemname', '$billdate', 'DC SUPPLIER OUT', 
			'BY DC SUPPLIER OUT (BILL NO: $billnumber )', '$billautonumber', '$billnumber', '$itemquantity', '$remarks', 
			...
			if ($itemcode != '')
			{
				$query7 = "insert into dcsupplieroutward_tax (bill_autonumber, billnumber, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into dcsupplieroutward_tax (bill_autonumber, billnumber, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname) 
	...
	header ("location:deliverychallansupplier1outward.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=PURCHASE");
	exit;
