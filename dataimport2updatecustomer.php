...
if ($upload == 'success')
{
	...
	$skipcountcustomercode = 0;
	$skipcountcustomername = 0;
	$skipcountaddress1 = 0;
	$skipcountaddress2 = 0;
	$skipcountarea = 0;
	$skipcountcity = 0;
	...
	$filename = $username."_tabdumpcustomer.txt";
	...
	foreach ( $splitcontents as $linecontent )
	{
		...
		if ($forloopcount > 1 && $forloopcount < $totallinecount) // to skip header row. to skip last empty row to avoid empty array error.
		{
			...
			$customercode = $splitdelimiter[1];
			$customername = $splitdelimiter[2];
			//echo "<br>";
			...
			$customercode = addslashes($customercode);
			...
			*/
			...
			$customercode = str_replace('\"\"', '"', $customercode); //To replace MS Excel given double quote to string.
			$customercode = str_replace('\"', '', $customercode); //To retain origina single char double quote and remeove Excel quote.
			$customername = str_replace('\"\"', '"', $customername); 
			$customername = str_replace('\"', '', $customername); 
			$address1 = str_replace('\"\"', '"', $address1); 
			$address1 = str_replace('\"', '', $address1); 
			...
			if ($customercode != '')
			{
				...
				if ($rowcount1 == 0)
				{
					$skipcountcustomercode = $skipcountcustomercode + 1;
				}
				else
				{
					$strlencustomercode = strlen($customercode);
					$startingcharacters = substr($customercode, 0, 3);
					if ($strlencustomercode != 11)
					{
						$skipcountcustomercode = $skipcountcustomercode + 1;
					}
					else 
					{
						if ($startingcharacters != 'CST')
						{
							$skipcountcustomercode = $skipcountcustomercode + 1;
						}
						else
						{
							if ($customername == '')
							{
								$skipcountcustomername = $skipcountcustomername + 1;
							}
							else
							{
								if ($address1 == '')
											...
											else
											{
												...
												else
												{
													...
													else
													{
														...
														$query2 = "update master_customer set customername = '$customername', address1 = '$address1', address2 = '$address2', 
														area = '$area', city = '$city', state = '$state', pincode = '$pincode' where customercode = '$customercode'";
						...
						} //For Customer Code Starting Character
					} //For Customer Code Length
				} //For Customer Code
			} //For Customer Code Not Empty
...
$totalerrorcount = $skipcountcustomercode + $skipcountcustomername + $skipcountaddress1 + $skipcountaddress2 + $skipcountarea + $skipcountcity + $skipcountstate + $skipcountpincode;
    ...
    <td width="97%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="860"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablebackgroundcolor1">
            <tr>
              <td><table width="83%" border="0" align="left" cellpadding="4" cellspacing="0" bordercolor="#666666" id="AutoNumber3" style="border-collapse: collapse">
                <tbody>
                  <tr bgcolor="#011E6A">
                    <td colspan="2" bgcolor="#CCCCCC" class="bodytext3"><strong>Customer Data Update From TAB Delimited File </strong></td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td align="left" valign="middle"  bgcolor="#FFFFFF" class="bodytext3">Error With Customer Code</td>
                    <td align="left" valign="top" bgcolor="#FFFFFF" ><?php echo $skipcountcustomercode; ?> <span class="bodytext3">( Please Check The Customer Code Exists In The Customer Masters ) </span></td>
                  </tr>
                  <tr>
                    <td align="left" valign="middle"  bgcolor="#FFFFFF" class="bodytext3">Error With Customer Name </td>
                    <td align="left" valign="top" bgcolor="#FFFFFF" ><?php echo $skipcountcustomername; ?> <span class="bodytext3">( Please Make Sure You Do Not Have Any Special Characters In Customer Name ) </span></td>
                  </tr>
                  <tr>
