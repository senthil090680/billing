...
if ($frm1submit1 == 'frm1submit1')
{
	...
	$query201 = "select * from master_salesreturn where billnumber = '$billnumber' and recordstatus <> 'DELETED' and companyanum = '$companyanum'";
	...
	{
		$query2 = "select max(billnumber) as maxbillnumber from master_salesreturn where companyanum = '$companyanum'";// order by auto_number desc limit 0, 1";
	...
	*/
	...
	$query3 = "insert into master_salesreturn (companyanum, billnumber, billdate, customertype, subtotal, customercode, customername, 
	...
	$query3 = "select auto_number from master_salesreturn  where companyanum = '$companyanum' and customercode = '$customercode' and lastupdate = '$lastupdate' and 
	...
	for ($i=1;$i<=1000;$i++)
	{
		...
		if ($serialnumber != '')
		{
			...
			$query4 = "insert into salesreturn_details (bill_autonumber, companyanum, 
			...
			if ($itemcode != '')
			{
				$query7 = "insert into salesreturn_tax (bill_autonumber, billnumber, billdate, itemanum, itemcode, itemname, 
		...
		while ($res72 = mysql_fetch_array($exec72))
		{
			...
			if ($rowcount72 != 0)
			{
				if ($itemcode != '')
				{
					$query8 = "insert into salesreturn_tax (bill_autonumber, billnumber, billdate, itemcode, itemname, itemrate, itemquantity, taxtype, 
					tax_autonumber, taxname, taxpercent, taxamount, amountaftertax, ipaddress, 
					updatedate, companyanum, companyname) 
...
*/	
	...
	header ("location:salesreturn1.php?src=frm1submit1&&st=success&&billnumber=$billnumber&&billautonumber=$billautonumber&&companyanum=$companyanum&&titlestr=SALES RETURN");
	exit;
